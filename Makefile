...
SHELL := /bin/bash
NAME := jx-project
BINARY_NAME := lighthouse-webui-plugin
BUILD_TARGET = build
MAIN_SRC_FILE=cmd/server/main.go
...
BUILDFLAGS :=  -ldflags \
  " -X $(ROOT_PACKAGE)/pkg/version.Version=$(VERSION)\
		...
		-X github.com/jenkins-x-plugins/lighthouse-webui-plugin/internal/version.Version=$(VERSION)\
		-X github.com/jenkins-x-plugins/lighthouse-webui-plugin/internal/version.Revision='$(REV)'\
		-X github.com/jenkins-x-plugins/lighthouse-webui-plugin/internal/version.Date='$(BUILD_DATE)'\
		$(BUILD_TIME_CONFIG_FLAGS)"
